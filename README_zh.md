...
AnshuDoc 是一款本地优先、基于知识图谱驱动的文档智能平台。它不只是一个可视化工具，而是你的 **“认知协同处理器”**。通过将图谱论（PageRank、社区检测）与现代大语言模型（LLM）深度融合，它能揭示你项目、研究和数据中隐藏的知识结构。

### 🎯 核心哲学
*   **知识复利**：区别于传统 RAG 每次查询时的重复检索，AnshuDoc 维护一个持久的、不断进化的知识层。
